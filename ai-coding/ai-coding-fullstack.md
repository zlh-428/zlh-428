...
本次建设「全栈交付 SKILL」 以 [Skill: rd-process-standardization（rd-process-standardization）](https://friday.sankuai.com/skills/skill-detail?activeTab=overview&activeTestTab=cases&id=2459)为基础

**全栈交付 SKILL：** [Skill: rd-process-fullstack（rd-process-fullstack）](https://friday.sankuai.com/skills/skill-detail?activeTab=overview&activeTestTab=cases&id=19606)作为一套覆盖全栈需求交付全流程的 Claude Code SKILL，集成了美团内部工具链（cargo-cli、code-cli、ones-cli、citadel 等），把从需求分析到泳道部署阶段串联成一套有状态、有约束的自动化流程。
...
全栈AI Coding交付SKILL rd-process-fullstack 的结构是三层：
...
2） **全栈场景下知识库的探索建设：**
- [MDP 规则仓库结构设计—工程级知识库的落地思考](https://km.sankuai.com/collabpage/2758407650)：当知识库足够完善时——不只是规范，而且包括完整的业务上下文、历史决策记录、技术约束的精确描述——AI 代理就具备了承担更大自主性的条件。它不再需要开发者在每个问题上手把手指导，而是能够从知识库中自主查询所需的上下文，做出符合项目特定约束的决策。
- [Skill: wiki-architect（wiki-architect-deep）](https://friday.sankuai.com/skills/skill-detail?activeTab=overview&activeTestTab=cases&id=16141)：用于沉淀业务领域知识、接口约束、架构决策，作为跨需求的持久化 Context 来源；SKILL 可以对现有代码库进行深度分析，自动提取服务地图、模块职责、依赖关系，生成结构化的知识文档，作为知识库建设的起点——比人工整理快得多，也更全面。
...
[T30进度管理优化-全栈复盘](https://km.sankuai.com/collabpage/2756604302)：这次实战以一个全栈存量需求为样本，涉及前端交互改造和后端查询逻辑变更，前后端物理上是两个独立工程目录。记录的重点不是业务本身，而是**这套 Harness 在实际交付中的运转方式和暴露出的问题**。
